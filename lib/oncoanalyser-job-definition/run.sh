...
Options:
  ...
  --prepare_reference_only    Prepare the reference

Environment:
  S3_REFDATA_URI              The uri to prepare reference data if using `--prepare_reference_only`.
                              This will use the `outdir` from the params file if not defined.
EOF
}
...
output_base=${S3_REFDATA_URI:-$(jq -r .outdir params.json)}
